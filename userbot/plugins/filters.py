...
async def remove_a_filter(r_handler):
    ...
    if not remove_filter(r_handler.chat_id, filt):
        await r_handler.edit("Filter` {} `tidak bisa.".format(filt))
    else:
        await r_handler.edit("Filter `{} `sudah dihapus".format(filt))
...
async def on_all_snip_delete(event):
    ...
    if filters:
        ...
        await edit_or_reply(event, f"filter di chat ini sudah dihapus")
    else:
        await edit_or_reply(event, f"Tidak ada filter di grup ini")


CMD_HELP.update(
    {
        "filters": "__**Nama Plugin:** Filters__\
    \n\n✅** CMD ➥** `.filters`\
    \n**Fungsi   ➥  **untuk melihat filter yang di gunakan pada userbot.\
    \n\n✅** CMD ➥** `.filter`  balas pesan .filter <keyword>\
    \n**Fungsi   ➥  **Untuk menyimpan filter 'katakunci'.\
    \nBot akan membalas pesan ketika katakunci muncul.\
    \nBisa pada media/stiker.\
    \n\n✅** CMD ➥** `.stop <keyword>`\
    \n**Fungsi   ➥  **Untuk menghapus katakunci.\
    \n\n✅** CMD ➥** `.rmfilters` \
    \n**Fungsi   ➥  **Hapus semua filter pada userbot."
    }
)
