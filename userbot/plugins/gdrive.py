...
CMD_HELP.update(
    {
        ...
        "\n\n✅** CMD ➥** `.greset`"
        "\n**Fungsi   ➥  **Setel ulang token Anda jika terjadi sesuatu yang buruk atau ubah akun drive."
        "\n\n✅** CMD ➥** `.ugd`"
        "\n**Fungsi   ➥  **Unggah file dari lokal atau uri / url / drivelink ke google drive."
        "\nuntuk drivelink itu diunggah hanya jika Anda mau."
        "\n\n✅** CMD ➥** `.gabort`"
        "\n**Fungsi   ➥  **Batalkan proses mengunggah atau mengunduh."
        "\n\n✅** CMD ➥** `.gdf mkdir`"
        "\n**Fungsi   ➥  **Buat folder gdrive."
        "\n\n✅** CMD ➥** `.gdf chck`"
        "\n**Fungsi   ➥  **Periksa file / folder di gdrive."
        "\n\n✅** CMD ➥** `.gdf rm`"
        "\n**Fungsi   ➥  **Hapus file / folder di gdrive."
        "\nTidak dapat diurungkan, melewatkan file sampah, jadi berhati-hatilah ..."
        "\n\n✅** CMD ➥** `.gdfset`"
        "\n**Fungsi   ➥  **Ubah direktori unggahan di gdrive."
        "\nke **G_DRIVE_FOLDER_ID** dan jika var itu kosong, maka unggahan akan menuju root."
        "\n\n✅** CMD ➥** `.gdfclear`"
        "\n**Fungsi   ➥  **hapus set parentId dari cmd\n>`.gdfset` "
        "\n\n✅** CMD ➥** `.gdl` <gdrive File-Tautan>"
        "\n**Fungsi   ➥  **Plugin Pengunduh File G-Drive Untuk Userbot.  Hanya file gdrive yang didukung sekarang"
        "\nGunakan flag `-u` untuk langsung mengunggah ke telegram di`.gdl` perintah"
        "\n\n✅** CMD ➥** `.glist`"
        "\n**Fungsi   ➥  **Dapatkan daftar folder dan file dengan ukuran default 50."
        "\nGunakan flags `-l range (1-1000)` untuk output batas."
        "\nGunakan flags `-p parents-folder_id` untuk daftar folder yang diberikan di gdrive."
        "\n\n***NOTE :**"
        "\nuntuk `.glist` Anda bisa menggabungkan flag -l dan -p dengan atau tanpa nama "
        "pada saat yang sama, ini harus menjadi flag `-l` terlebih dahulu sebelum menggunakan flag` -p`.\n"
        "Dan secara default daftar dari 'waktu edit' terbaru dan kemudian folder."
    }
)
